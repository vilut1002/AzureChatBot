...
namespace Pibot.Dialogs
{
    public class BookingDialog : ComponentDialog
    {
        ...
        {
            var notes = $"※ 헌혈 예약 시 유의사항 ※{System.Environment.NewLine}";
            notes += $"- 6개월 후까지 예약할 수 있으며, 당일 예약은 불가합니다.{System.Environment.NewLine}";
            notes += $"- 최근 헌혈혈액검사에 따라 헌혈이 제한될 수 있습니다.{System.Environment.NewLine}";
            notes += $"- 예약시간 경과 시 예약이 취소되니 주의해 주십시오.{System.Environment.NewLine}";
            notes += $"- 헌혈의 집 도착 시 예약헐혈자임을 직원에게 말씀해 주십시오.{System.Environment.NewLine}";
        ...
        private async Task<DialogTurnResult> HouseChoiceStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            new PromptOptions
            {
                Prompt = MessageFactory.Text("어느 지점으로 방문하시겠어요?"),
        ...
        private async Task<DialogTurnResult> ConfirmStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            bookingResult += $"성별 : {userProfile.Sex}{System.Environment.NewLine}";
            bookingResult += $"나이 : {userProfile.Age}{System.Environment.NewLine}";
            bookingResult += $"연락처 : {userProfile.Phone}{System.Environment.NewLine}";
            bookingResult += $"지점 : {bookingDetails.House}{System.Environment.NewLine}";
