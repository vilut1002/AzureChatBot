    ...
    public class QuizDialog : ComponentDialog
    {
        ...
        private static async Task<DialogTurnResult> Q1StepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            return await stepContext.PromptAsync(nameof(ChoicePrompt),
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("Q1. 헌혈 증서를 구입할 수 있다?"),
                    Choices = ChoiceFactory.ToChoices(new List<string> { "O", "X"}),
                }, cancellationToken);
        }
        ...
        {
            if ( "X"== ((FoundChoice)stepContext.Result).Value)
            {
                score++;
            }
            var msg = "정답은 X 입니다. \n\n" +
            ...
            return await stepContext.PromptAsync(nameof(ChoicePrompt),
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("Q2. 헌혈을 하는 것은 건강에 나쁘다?"),
                    Choices = ChoiceFactory.ToChoices(new List<string> { "O", "X" }),
                }, cancellationToken);
        }
        ...
        private static async Task<DialogTurnResult> Q3StepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            return await stepContext.PromptAsync(nameof(ChoicePrompt),
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("Q3. 헌혈을 통해 에이즈 등 질병에 감염될 수 있다?"),
                    Choices = ChoiceFactory.ToChoices(new List<string> { "O", "X" }),
                }, cancellationToken);
        }
        ...
        private static async Task<DialogTurnResult> Q4StepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            var msg = "정답은 X 입니다. \n\n" +
                      "- 헌혈과정은 매우 안전합니다. 헌혈에 사용되는 모든 기구(바늘, 혈액백 등)은 무균처리되어 있으며, 한번 사용 후에는 모두 폐기처분 하기 때문에 헌혈로 인해 에이즈등 다른질병에 감염될 위험은 전혀 없습니다.";
            await stepContext.Context.SendActivityAsync(MessageFactory.Text(msg), cancellationToken);
            return await stepContext.PromptAsync(nameof(ChoicePrompt),
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("Q4. 헌혈을 하면 살이 빠지고 다이어트에도 도움이 된다?"),
                    Choices = ChoiceFactory.ToChoices(new List<string> { "O", "X" }),
                }, cancellationToken);
        }
        ...
        private static async Task<DialogTurnResult> Q5StepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            var msg = "정답은 X 입니다. \n\n" +
                      "- 헌혈을 하면 헌혈량 만큼이 체외로 빠져나오는 것은 사실이지만 조직에 있던 혈액이 혈관 내로 바로 이동하여 보상하며, 이후 며칠 또는 몇 주간 음식 및 수분 섭취 등으로 원래 상태로 보충됩니다. \n\n" +
                      "- 따라서 헌혈은 다이어트와는 무관합니다.";
            await stepContext.Context.SendActivityAsync(MessageFactory.Text(msg), cancellationToken);
            return await stepContext.PromptAsync(nameof(ChoicePrompt),
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("Q5. 헌혈을 해도 빈혈에 걸리지 않는다?"),
                    Choices = ChoiceFactory.ToChoices(new List<string> { "O", "X" }),
                }, cancellationToken);
        }
        ...
        {
            if ("O" == ((FoundChoice)stepContext.Result).Value)
            {
                score++;
            }
            var msg = "정답은 O 입니다. \n\n" +
                      "- 헌혈은 자기 몸에 여유로 가지고 있는 혈액을 나눠주는 것으로, 헌혈 전에 충분한 혈액이 있는지를 판단하기 위해 적혈구 내의 혈색소(헤모글로빈) 치를 측정합니다. 따라서 헌혈로 빈혈에 걸리지는 않습니다. 또한 헌혈자를 보호하기 위하여 연간 헌혈가능 횟수도 전혈헌혈은 5회로 제한하고 있습니다.";
            await stepContext.Context.SendActivityAsync(MessageFactory.Text(msg), cancellationToken);
            return await stepContext.PromptAsync(nameof(ChoicePrompt),
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("Q6. 헌혈을 많이 하면 혈관이 좁아진다?"),
                    Choices = ChoiceFactory.ToChoices(new List<string> { "O", "X" }),
                }, cancellationToken);
        }
        ...
        private static async Task<DialogTurnResult> Q7StepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            return await stepContext.PromptAsync(nameof(ChoicePrompt),
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("Q7. 전에 헌혈을 하지 못한 사람도 헌혈이 가능하다? "),
                    Choices = ChoiceFactory.ToChoices(new List<string> { "O", "X" }),
                }, cancellationToken);
        }
        ...
        private static async Task<DialogTurnResult> Q8StepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            var msg = "정답은 O 입니다. \n\n" +
                      "- 일시적으로 헌혈에 참여하지 못한 경우, 다시 헌혈에 참여하실 수 있습니다. \n\n" +
            ...
            return await stepContext.PromptAsync(nameof(ChoicePrompt),
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("Q8. 헌혈을 하면 에이즈검사도 진행하지만 검사 결과를 통보해주지는 않는다?"),
                    Choices = ChoiceFactory.ToChoices(new List<string> { "O", "X" }),
                }, cancellationToken);
        }
        ...
        private static async Task<DialogTurnResult> Q9StepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            var msg = "정답은 O 입니다. \n\n" +
                      "- 헌혈한 혈액은 혈액형검사, B형간염 항원검사, C형간염 항체검사, ALT검사, 매독항체검사, HIV검사를 실시하나 에이즈 검사를 목적으로 헌혈에 참여하는 것을 막기 위해서 에이즈(HIV) 검사결과는 통보해 주지 않습니다. " +
                      "- 현재 각 구청 보건소에서는 개인의 비밀을 보장하면서 에이즈 검사를 실시하고 있으니 참고바랍니다.";
            await stepContext.Context.SendActivityAsync(MessageFactory.Text(msg), cancellationToken);
            return await stepContext.PromptAsync(nameof(ChoicePrompt),
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("Q9. 나의 헌혈기록이나 검사결과에 대한 비밀보장이 어렵다?"),
                    Choices = ChoiceFactory.ToChoices(new List<string> { "O", "X" }),
                }, cancellationToken);
        }
        ...
        private static async Task<DialogTurnResult> Q10StepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            var msg = "정답은 X 입니다. \n\n" +
                      "- 헌혈자의 모든 헌혈기록이나 검사결과는 비밀이 보장되며, 본인이 아닌 다른 분들에게는 공개되지 않도록 법적으로 보호됩니다. \n\n " +
            ...
            return await stepContext.PromptAsync(nameof(ChoicePrompt),
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("Q10. 적십자사는 혈액 사업을 통해 사익과 이윤창출을 추구한다?"),
                    Choices = ChoiceFactory.ToChoices(new List<string> { "O", "X" }),
                }, cancellationToken);
        }
